...
import { useDeleteManyAssets } from "@/hooks/data/useAssests";
...
const DeleteManyAssetModal = ({
	open,
	setOpen,
	selectedIds,
	onSuccess,
}: Props) => {
	const { mutate: deleteMany, isPending } = useDeleteManyAssets();
	...
	const handleConfirm = () => {
		...
		deleteMany(idsNumeric, {
			onSuccess: () => {
				toast.success(`Đã xóa ${selectedIds.length} tài sản`);
				setOpen(false);
...
export default DeleteManyAssetModal;
