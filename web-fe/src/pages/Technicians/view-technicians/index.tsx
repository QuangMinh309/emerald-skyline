...
import { technicianColumns } from "./columns";
				...
				<PageHeader
					...
					actions={
						<div className="flex items-center gap-2">
							...
							<ActionDropdown
								options={actions}
								sampleFileUrl="/template/technician_import_template.xlsx"
							/>
						</div>
					}
				/>
					...
					{isLoading ? (
						...
						<CustomTable
							...
							onDelete={(row) => {
								...
								setSelectedTechnician(row as Technician);
							}}
							onView={(row) => navigate(`/technicians/${row.id}`)}
						/>
					)}
				</div>
			</div>
