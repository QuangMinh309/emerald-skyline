...
import { useDeleteManyResidents } from "@/hooks/data/useResidents";
...
const DeleteManyResidentModal = ({
	open,
	setOpen,
	selectedIds,
	onSuccess,
}: Props) => {
	const { mutate: deleteMany, isPending } = useDeleteManyResidents();
	...
	const handleConfirm = () => {
		...
		deleteMany(idsNumeric, {
			onSuccess: () => {
				toast.success(`Đã xóa ${selectedIds.length} cư dân`);
				setOpen(false);
...
export default DeleteManyResidentModal;
