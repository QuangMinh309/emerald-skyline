...
import { useDeleteManyMaintenanceTickets } from "@/hooks/data/useMaintenance";
...
const DeleteManyMaintenanceModal = ({
	open,
	setOpen,
	selectedIds,
	onSuccess,
}: Props) => {
	const { mutate: deleteMany, isPending } = useDeleteManyMaintenanceTickets();
	...
	const handleConfirm = () => {
		...
		deleteMany(idsNumeric, {
			onSuccess: () => {
				toast.success(`Đã xóa ${selectedIds.length} phiếu bảo trì`);
				setOpen(false);
...
export default DeleteManyMaintenanceModal;
